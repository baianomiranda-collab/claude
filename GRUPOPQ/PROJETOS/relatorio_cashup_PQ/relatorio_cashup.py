...
Fluxo 100% automatico, sem pausa manual:
  ...
  7. Encaminha esse email (sem recompor) para os destinatarios finais
     em EMAIL_PARA_PQ (.env, lista separada por virgula)
...
EMAIL_PARA_LIST = [e.strip() for e in (os.getenv("EMAIL_PARA_PQ") or "").split(",") if e.strip()]
...
def verificar_ambiente() -> bool:
    ...
    if not EMAIL_PARA_LIST:
        print("  ERRO: EMAIL_PARA_PQ nao tem nenhum destinatario valido")
        return False
